...
from .option import gen_options
import ipdb
...
class TumFormatDataLoader(RegistrationDataset):
    ...
    def get_data_pair(self, scene_id, images_list, depths_list, poses_list, intrinsics_list, id1, id2):
        ...
        ### convert to torch
        shuffle1 = np.random.permutation(xyz1.shape[0])
        shuffle2 = np.random.permutation(xyz2.shape[0])
        in_dict = {
            'pc1': (xyz1.astype(np.float32))[shuffle1],
            'pc2': (xyz2.astype(np.float32))[shuffle2],            
            'color1': (color_pc1.astype(np.float32))[shuffle1],
            'color2': (color_pc2.astype(np.float32))[shuffle2],
            'T': (pose_f1_to_f2.astype(np.float32)),
            't': (pose_f1_to_f2[:3, 3].astype(np.float32)),
            'R': (pose_f1_to_f2[:3, :3].astype(np.float32)),
            'seq': scene_id,
            'pc1_id': id1,
            'pc2_id': id2,
            'intrinsic': K,
    ...
    def get_dataindex(self, dataset_index, index):
        ...
        oses_list = self.scene_info[scene_id]['poses']
